...
def fetch_historical_data(record_count):
    ...
    while total_records_fetched < record_count:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[{current_time}] Fetched page {page}, total records: {total_records_fetched}")

                if total_records_fetched >= record_count or len(records) < 25:
                    break
                page += 1
                time.sleep(random.uniform(1, 3))  # Tránh bị phát hiện là bot
            elif response.status_code in [401, 403, 429]:
                if response.status_code in [403, 429]:
                    print(f"IP restricted (status: {response.status_code}), renewing Tor IP...")
                    if renew_tor_ip():
                        time.sleep(10)
                    else:
                        time.sleep(60)
    ...
    # Lưu dữ liệu vào CSV
    if all_records:
        file_exists = os.path.exists("results_history.csv")
        try:
            with open("results_history.csv", "a", newline="") as f:
                writer = csv.writer(f)
                if not file_exists:
                    writer.writerow(["issue", "issueDate", "planDrawTime", "color", "value"])
                writer.writerows(all_records)
            print(f"[{current_time}] Saved {total_records_fetched} records to results_history.csv")
        except Exception as e:
...
if __name__ == "__main__":
    try:
        record_count = int(input("Enter the number of historical records to fetch (e.g., 50): "))
        if record_count <= 0:
            print("Please enter a positive number.")
        else:
            print(f"Fetching {record_count} historical records...")
            fetch_historical_data(record_count)
    except ValueError:
